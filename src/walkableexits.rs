...
pub fn get_walkable_exits(seed_data_json: &mut SeedData) {
    seed_data_json.levels.iter_mut()
        .map(|o| o.objects.iter_mut()
            .map(|e| if e.object_type == "exit" {
                e.x = e.x + o.offset.x;
                e.y = e.y + o.offset.y;
            })
        );
...
fn get_valid_exits<'a>(level_data: &'a mut LevelData, exits: &'a mut Vec<Object>) -> &'a mut Vec<Object> {
    ...
    if level_data.map[1].len() > 3 {
        ...
        if exit2 > 2 && exit2 < 50 {
            let x: u32 = level_data.offset.x + (level_data.map[1][0] + level_data.map[1][1] + level_data.map[1][2]) as u32 + (level_data.map[1][3] as f32 / 2.) as u32;
            let y: u32 = level_data.offset.y;
            exits.push(Object::new_exit(x, y, level_data.id, &exits));
    ...
    if level_data.map[level_data.map.len() - 1].len() > 1 {
        ...
        if exit1 > 2 && exit1 < 50 {
            let x: u32 = level_data.offset.x + (level_data.map[level_data.map.len() - 1][0]) as u32 + (level_data.map[level_data.map.len() - 1][1] as f32 / 2.) as u32;
    ...
    if level_data.map[level_data.map.len() - 1].len() > 3 {
        ...
        if exit2 > 2 && exit2 < 50 {
            let x: u32 = level_data.offset.x + (level_data.map[level_data.map.len() - 1][0] + level_data.map[level_data.map.len() - 1][1] + level_data.map[level_data.map.len() - 1][2]) as u32 + (level_data.map[level_data.map.len() - 1][3] as f32 / 2.) as u32;
            let y: u32 = level_data.offset.y + level_data.size.height;
            exits.push(Object::new_exit(x, y, level_data.id, &exits));
